...
func NotLoggedIn(c *router.Context) bool {
	if len(c.User) == 0 {
...
func HandleSessions(c *router.Context, second, third string) {
	if second == "" {
...
func handleSessionsIndex(c *router.Context) {
	if c.Method == "DELETE" {
		DestroySession(c)
...
func CreateSession(c *router.Context) {
...
func DestroySession(c *router.Context) {
	id := c.User["id"].(int64)
