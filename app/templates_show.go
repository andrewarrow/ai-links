...
func handleTemplateShowPost(c *router.Context, guid string) {
	cols, editable := GetCols(c, "template")
	list := []string{}
	...
	c.ValidateUpdate("template")
	message := c.ValidateUpdate("template")
	returnPath := "/sd/templates"
	if message != "" {
		router.SetFlash(c, message)
		http.Redirect(c.Writer, c.Request, returnPath+"/"+guid, 302)
		return
	}
	message = c.Update("template", "where guid=", guid)
	if message != "" {
		router.SetFlash(c, message)
		http.Redirect(c.Writer, c.Request, returnPath+"/"+guid, 302)
		return
...
func handleTemplateShow(c *router.Context, guid string) {
	template := c.One("template", "where guid=$1", guid)
	...
	m["headers"] = headers
	m["cells"] = c.MakeCells(util.ToAny(cols), headers, params, "_template_show")
	m["col_attributes"] = colAttributes
	m["save"] = true
	//topVars := map[string]any{}
	//topVars["name"] = template["name"]
	//topVars["guid"] = guid
	send := map[string]any{}
	send["bottom"] = c.Template("table_show.html", m)
	//send["top"] = c.Template("templates_top.html", topVars)
